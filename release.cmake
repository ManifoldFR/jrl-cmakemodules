  ...
  if(UNIX)
    ...
    add_custom_target(
      ...
      COMMAND
        ...
        (
          ${GIT}
          add pixi.toml && ${GIT} commit -m
          "release: Update pixi.toml version to $$VERSION" && echo
          "Updated pixi.toml and committed"
        )
        ; fi
      # cmake-format: on
    )
    ...
    add_custom_target(
      ...
      COMMAND
        ...
        (${BUILD_CMD} ${PROJECT_NAME}-release_package_xml) ; fi &&
        # Update version in pyproject.toml if it exists
        if [ -f "pyproject.toml" ]; then
        (${BUILD_CMD} ${PROJECT_NAME}-release_pyproject_toml) ; fi &&
        # Update CHANGELOG.md if it exists
        if [ -f "CHANGELOG.md" ]; then
        (${BUILD_CMD} ${PROJECT_NAME}-release_changelog) ; fi &&
        # Update version in pixi.toml if it exists
        if [ -f "pixi.toml" ]; then
        (${BUILD_CMD} ${PROJECT_NAME}-release_pixi_toml) ; fi &&
        # Update date and version in CITATION.cff if it exists
        if [ -f "CITATION.cff" ]; then
        (${BUILD_CMD} ${PROJECT_NAME}-release_citation_cff) ; fi && ${GIT} tag
        -s v$$VERSION -m "Release of version $$VERSION." && cd
        ${PROJECT_BINARY_DIR} && cmake ${PROJECT_SOURCE_DIR} && ${BUILD_CMD}
        ${PROJECT_NAME}-distcheck ||
        (
          echo
          "Please fix distcheck first." && cd ${PROJECT_SOURCE_DIR} && ${GIT}
          tag -d v$$VERSION && cd ${PROJECT_BINARY_DIR} && cmake
          ${PROJECT_SOURCE_DIR} && false
        )
        && ${BUILD_CMD} ${PROJECT_NAME}-dist && ${BUILD_CMD}
        ${PROJECT_NAME}-distclean && echo
